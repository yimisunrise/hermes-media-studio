...
## 4. Verification (manual — requires browser)

- [ ] 4.1 Verify extension loads without errors in WebUI console (reload WebUI page, check console)
- [ ] 4.2 Verify setup dialog appears on first load (workspace not initialized, shows dir list + Initialize/Cancel)
- [ ] 4.3 Verify directory creation succeeds after user confirmation (click Initialize, watch progress)
- [ ] 4.4 Verify all 8 views render correctly after initialization (navigate each tab)
- [ ] 4.5 Verify extension skips initialization dialog on subsequent loads (reload page, should go straight to views)
- [ ] 4.6 Verify API calls use correct endpoints and session_id with `lsp_diagnostics` clean
