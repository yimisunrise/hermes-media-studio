...
Define and enforce consistent sizing, spacing, and border-radius for all interactive form elements (input, select, textarea, button) across every view in Hermes Media Studio. All styling SHALL be driven by a shared set of CSS custom properties in `framework/app.css`, with zero inline style overrides for sizing or corner-rounding.
...
- `--ms-input-padding`: padding for input/select/textarea/button, defaulting to `6px 12px`
