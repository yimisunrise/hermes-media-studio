...
│  │                                                               │   │
│  │  AgentTaskPoller                                              │   │
│  │  ├── createTask / readIndex / updateTaskStatus                │   │
│  │  ├── readBrief / readResult / writeResult                     │   │
│  │  └── .agent-tasks/ 目录通信协议 (扩展侧)                       │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
```
...
│
├── .agent-tasks/                      ← 【代理层】Hermes-Agent 任务通信协议
...
class AgentTaskPoller {
  ...
  async readIndex()                        // 读 .agent-tasks/index.json 返回全量状态
  async updateTaskStatus(uuid, status)     // 更新 index.json 中的任务状态 + processing 锁
  async createTask(type, brief, files)     // 创建 .agent-tasks/<uuid>/ + job.json + brief.md
  async readBrief(uuid)                    // 读 .agent-tasks/<uuid>/brief.md
  async readResult(uuid)                   // 读 .agent-tasks/<uuid>/result.md
  async writeResult(uuid, content)         // 写 result.md + 自动标记 done
}
```

**职责**: **仅负责传输层**——任务创建、状态管理、结果读写。不解析业务内容，不关心 brief.md/result.md 的具体字段。业务层的任务类型注册、结果分发由 `business/agent/` 实现。

**协议边界**:
- 创建任务时写入 job.json（薄元数据）+ 更新 index.json，返回 UUID
- readIndex() 返回 index.json 全量状态，业务层据此判断流转
- readResult() 返回 result.md 原始文本，不负责解析 frontmatter
- 结果清理由扩展下次初始化时按需处理，业务层无需关心
...
| Agent 通信目录 | `.agent-tasks/`（dot-dir 隐藏） | 与 `.database/` / `.index/` 保持一致 |
...
| Agent 防重复拾取 | index.json processing 字段锁 | 集中状态管理避免目录 mv 的复杂性 |
